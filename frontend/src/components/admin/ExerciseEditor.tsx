...
export const initialDefinitions: Record<ExerciseTypeCode, () => ExerciseDefinition> = {
  ...
  fill_blanks: () => ({items:[{id:'sentence-1',parts:[{kind:'text',text:''},{kind:'blank',id:'blank-1',hint:null,accepted_answers:['']}]}]}),
}
...
function FillBlanksEditor({ definition, onChange, errors }: EditorProps<FillBlanksDefinition>) {
  ...
  const fieldErrors = (path: string) => errors[path] ?? []
  return <div className="space-y-4"><FieldErrors errors={fieldErrors('definition.items')} />{definition.items.map((item, itemIndex) => <div key={`${item.id}-${itemIndex}`} className="rounded border p-3"><div className="flex gap-2"><Input value={item.id} placeholder="ID элемента" onChange={event => updateItem(itemIndex, value => ({...value,id:event.target.value}))}/><Button size="sm" variant="secondary" disabled={itemIndex===0} onClick={() => onChange({...definition,items:move(definition.items,itemIndex,itemIndex-1)})}>↑</Button><Button size="sm" variant="secondary" disabled={itemIndex===definition.items.length-1} onClick={() => onChange({...definition,items:move(definition.items,itemIndex,itemIndex+1)})}>↓</Button><Button size="sm" variant="secondary" onClick={() => onChange({...definition,items:definition.items.filter((_,index) => index !== itemIndex)})}>Удалить элемент</Button></div><FieldErrors errors={fieldErrors(`definition.items.${itemIndex}.id`)} />{item.parts.map((part, partIndex) => { const path = `definition.items.${itemIndex}.parts.${partIndex}`; const partKey = part.kind === 'blank' ? part.id : partIndex; return <div className="mt-2 rounded bg-gray-50 p-2" key={`${part.kind}-${partKey}-${partIndex}`}><div className="flex gap-2">{part.kind === 'text' ? <Input value={part.text} placeholder="Текст" onChange={event => updateItem(itemIndex, value => ({...value,parts:value.parts.map((current,index) => index === partIndex ? {...current,text:event.target.value} : current)}))}/> : <div className="grid flex-1 gap-2 md:grid-cols-3"><Input value={part.id} placeholder="ID пропуска" onChange={event => updateItem(itemIndex, value => ({...value,parts:value.parts.map((current,index) => index === partIndex ? {...current,id:event.target.value} : current)}))}/><Input value={part.hint ?? ''} placeholder="Подсказка" onChange={event => updateItem(itemIndex, value => ({...value,parts:value.parts.map((current,index) => index === partIndex ? {...current,hint:event.target.value || null} : current)}))}/><Input value={part.accepted_answers.join(', ')} placeholder="Ответы через запятую" onChange={event => updateItem(itemIndex, value => ({...value,parts:value.parts.map((current,index) => index === partIndex ? {...current,accepted_answers:event.target.value.split(',').map(answer => answer.trim())} : current)}))}/></div>}<Button size="sm" variant="secondary" disabled={partIndex===0} onClick={() => updateItem(itemIndex,value => ({...value,parts:move(value.parts,partIndex,partIndex-1)}))}>↑</Button><Button size="sm" variant="secondary" disabled={partIndex===item.parts.length-1} onClick={() => updateItem(itemIndex,value => ({...value,parts:move(value.parts,partIndex,partIndex+1)}))}>↓</Button><Button size="sm" variant="secondary" onClick={() => updateItem(itemIndex,value => ({...value,parts:value.parts.filter((_,index) => index !== partIndex)}))}>Удалить</Button></div><FieldErrors errors={Object.entries(errors).filter(([errorPath]) => errorPath.startsWith(path)).flatMap(([, messages]) => messages)} /></div>})}<div className="mt-2 flex gap-2"><Button size="sm" onClick={() => updateItem(itemIndex,value => ({...value,parts:[...value.parts,{kind:'text',text:''}]}))}>Текст</Button><Button size="sm" onClick={() => updateItem(itemIndex,value => ({...value,parts:[...value.parts,{kind:'blank',id:'',hint:null,accepted_answers:['']}]}))}>Пропуск</Button></div></div>)}<Button size="sm" onClick={() => onChange({...definition,items:[...definition.items,{id:'',parts:[{kind:'text',text:''}]}]})}>Добавить элемент</Button></div>
}
...
export function ExerciseEditor({ type, definition, onChange, errors = {} }: {type: ExerciseTypeCode; definition: ExerciseDefinition; onChange: (value: ExerciseDefinition) => void; errors?: ExerciseFieldErrors}) {
  const Editor = editorRegistry[type]
  return <Editor definition={definition} onChange={onChange} errors={errors} />
}
