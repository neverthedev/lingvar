...
из сессии. Каталог отображает возвращённые backend группы по корневым правилам.
Для соседних пропусков без подсказок renderer показывает одно поле, отправляет
одну групповую проверку и после ответа заново собирает значение из отдельных
серверных состояний, включая раскрытый ответ исчерпанного пропуска.
...
JSON-редактора нет. Форма загружает лес правил, требует правило упражнения и для
`fill_blanks` ограничивает необязательное правило пропуска выбранной ветвью.
...
содержимое `GET /api/exercises/{slug}/content`. Каталог сгруппирован по корневому
правилу назначенного упражнению узла: группы следуют порядку корней, упражнения
внутри — `display_order` и ID. Общий envelope содержимого содержит метаданные
...
`numerators`; для `fill_blanks` удаляет `accepted_answers` и правила пропусков из
учебной проекции. Максимальная последовательность соседних пропусков без подсказок
представляется как `blank_group` с ID и числом слов каждого ответа.
...
ведётся независимо по пропускам, а hint показывается после поля в скобках.
`fill_blank_group_check` атомарно делит общий ответ по сохранённым длинам,
обновляет только открытые пропуски и один раз увеличивает revision; лишние слова
игнорируются, а недостающий фрагмент считается ошибкой своего пропуска. Визуальные
...
`status`, а `rule_id` обязателен и изменяем. Definition валидируется выбранной
Pydantic-схемой до записи, неизвестная
пара тип/версия и ошибки структуры возвращают структурированный `422`, повторный
slug — `409`. Для `fill_blanks` дополнительная проверка по БД допускает у пропуска
только правило упражнения или его потомка; мутации упражнений и дерева правил
сериализуются общей блокировкой `rules` до commit. Удаления упражнений нет. Страница `/admin` перенаправляет в
...
Перенос также откатывается, если после него правило пропуска выходит из ветви
правила упражнения. Связанное с упражнением или пропуском правило удалить нельзя;
иначе удаление одним commit сначала SQL-операциями переносит прямых потомков к
прежнему родителю (или в корень), затем удаляет узел. Rollback отменяет обе части.
...
| `exercises` | Уникальный slug, обязательный `rule_id → rules.id` с запретом удаления правила, тип и версия схемы, метаданные каталога, `draft`/`published` и строго валидируемый `definition` JSONB. Словарные definition хранят `source_code` и настройки, но не копии слов; пропуск `fill_blanks` может хранить необязательный `rule_id` своей ветви. |
...
временные сессии упражнений не являются долговременной историей прохождений.
Упражнение связано с одним правилом, а отдельные пропуски могут уточнять его
подветвь; учебный каталог читает дерево только для группировки по корню. Нет таблиц
уроков/блоков, долговременной истории сессионных ответов и персонального прогресса
по частям правил. Дерево `Rule` изменяется только через административный контур.
Согласованная
...
downgrade также запрещён. [Revision связей с правилами](../backend/migrations/versions/0004_exercise_rule_mapping.py)
добавляет обязательный FK упражнения: при наличии прежних упражнений создаёт
корневое правило «Правила польского языка», назначает его строкам без правила и
не меняет их definition; downgrade запрещён. [CLI](../backend/migrate.py) содержит
...
схему, последовательное применение четырёх revisions, точный backfill каталога и
переход `0002 → 0003` с backfill существующей сессии, её публичного ID, revision
и фиксированного TTL, а также переход к `0004` с сохранением definition и
назначением обязательного правила,
...
TTL и restart. Он также проверяет валидность связей упражнений/пропусков с деревом,
rollback запрещённых delete/move, группировку каталога и атомарное распределение
слов между объединёнными пропусками. Сценарий правил в user workflow
...
Он же проверяет группы каталога и общее поле соседних пропусков, включая
частичный результат, неизменный blur и раскрытие ответов. Административный
browser-сценарий проверяет обязательный выбор правила и фильтрацию подветви
пропуска. Playwright использует одного worker, потому что сценарии разделяют одну
одноразовую тестовую схему PostgreSQL.
